module.exports = async (req, res) => {
  try {
    ...
    const userSnap = await services.db.ref(`eyobShopy/users/${decoded.uid}`).once("value");
    const user = userSnap.val() || {};
    const name = String(user.name || "Customer");

    const ownerSnap = await services.db.ref("eyobShopy/pushTokens/owner").once("value");
